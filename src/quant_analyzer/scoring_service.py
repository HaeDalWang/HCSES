...
DynamoDB 조회 + ScoringContext 빌드 + 20거래일 누적합 계산
"""
import json
import logging
import os
...
def load_market_indicators(target_date: date) -> dict:
    ...
    당일 데이터 없으면 전일 데이터 fallback — 단, 지연 지표임을 WARNING 로그로 기록.
    지연 지표 사용 시 Kill-Switch 기준을 보수적으로 강화 (stale=True 플래그 포함).
    """
...
def _get_prev_rsi(ticker: str, target_date: date) -> Optional[float]:
    """전일 RSI 조회 (Momentum Pivot 감지용). 주말+공휴일 건너뛰기."""
    prev_date = target_date - timedelta(days=1)
    # 주말 및 공휴일 건너뛰기 (최대 7일 탐색)
    for _ in range(7):
        if prev_date.weekday() < 5:  # 평일
...
def _calc_cumulative_net_buy(ticker: str, target_date: date) -> tuple[Optional[float], Optional[float]]:
    """
    BR-09: 최근 20거래일 외국인+기관 누적 순매수합 계산
    반환: (today_cumulative, prev_cumulative)
    """
    table = db.get_dynamodb().Table(STOCK_DAILY_TABLE)
    try:
        # 최근 22거래일 조회 (주말 포함 여유)
        ...
        today_cum = _sum(recent) if recent else None
        prev_cum = _sum(prev_20) if prev_20 else None
        return today_cum, prev_cum
    except ClientError as e:
...
def build_scoring_context(
    ...
    """DynamoDB 레코드 + 통계 → ScoringContext 빌드"""
    pbr_min_value = None
    if stats:
